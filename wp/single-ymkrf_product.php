...
if ( $cat && $cat->slug === 'pack4' ) {
	include get_stylesheet_directory() . '/ymkrf-pack4-plan.php';
	continue;
}
  ...
  <div class="l-wrap">
    ...
    <?php if ( $d['points'] ) : ?>
      <ul class="p-prd__points">
        <?php
        /* 特徴のアイコン。
           もとは 円マーク／収納／キラキラ の3種類でしたが、
           書かれている内容と合わないことが多いので、ぜんぶキラキラにしました
           （2026/09/23 ユーザー指示「内容と合っていないから、全て3番目の✨にして」）。 */
        $star = '<path d="M12 3l1.6 4.4L18 9l-4.4 1.6L12 15l-1.6-4.4L6 9l4.4-1.6z"/>'
              . '<path d="M18.5 15.5l.7 1.9 1.9.7-1.9.7-.7 1.9-.7-1.9-1.9-.7 1.9-.7z'
              . 'M5.5 15.5l.6 1.5 1.5.6-1.5.6-.6 1.5-.6-1.5L3.4 18l1.5-.6z"/>';
        $icons = array( $star, $star, $star );
