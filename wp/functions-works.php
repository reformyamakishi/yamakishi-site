...
function ymkrf_works_parts_master() {
	...
	return array(
		...
		'interior'   => array( 'クロス・床',          'クロス・床リフォーム事例' ),
		'renovation' => array( '内装・改装',          '内装・改装の事例' ),
		'window'     => array( '窓・断熱',            '窓・断熱リフォーム事例' ),
		'door'       => array( 'ドア',                'ドア交換の事例' ),
		/* ベランダ・サンルーム・風除室・テラスは、同じ仲間なので
		   「エクステリア」にまとめました（2026/09/08 ユーザー指示） */
		'exterior'   => array( 'エクステリア',        'エクステリア工事の事例' ),
		'carport'    => array( 'カーポート',          'カーポート設置の事例' ),
		'storage'    => array( '物置',                '物置設置の事例' ),
		'fence'      => array( 'フェンス・門まわり',  'フェンス工事の事例' ),
...
add_action( 'admin_init', function () {
	if ( get_option( 'ymkrf_works_cat_ver' ) === '10' ) return;
	...
	/* ベランダ・サンルーム／風除室／テラス は「エクステリア」にまとめました。
	   すでに付いている事例を、こちらへ移してから片づけます。（2026/09/08） */
	$to = get_term_by( 'slug', 'exterior', 'ymkrf_works_cat' );
	if ( $to && ! is_wp_error( $to ) ) {
		foreach ( array( 'veranda', 'windbreak', 'terrace' ) as $slug ) {
			$from = get_term_by( 'slug', $slug, 'ymkrf_works_cat' );
			if ( ! $from || is_wp_error( $from ) ) continue;
			...
			foreach ( $ids as $pid ) {
				wp_set_object_terms( $pid, (int) $to->term_id, 'ymkrf_works_cat', true );
				wp_remove_object_terms( $pid, (int) $from->term_id, 'ymkrf_works_cat' );
			}
			wp_delete_term( $from->term_id, 'ymkrf_works_cat' );
		}
	}
	...
	update_option( 'ymkrf_works_cat_ver', '10' );
} );
...
function ymkrf_works_prod_groups_master() {
	/* トップページの「リフォームメニュー」と、名前も順番もそろえてあります。
	   （2026/09/08 ユーザー指示「すべてこのサイト内、リフォームメニューに
	     合わせてほしい」）
	   「手洗い付きカウンタートイレ」だけは、トイレの中の分けかたです。 */
	return array(
		'kitchen'        => 'キッチン',
		'bathroom'       => 'お風呂',
		'toilet'         => 'トイレ',
...
/* ------------------------------------------------------------
   ...
   給湯器・エコキュート・外壁・屋根・窓・玄関ドア・内装・改装は、
   品番が年ごとに変わったり、そもそも品番で選ぶものではありません。
   施工事例では「これを使いました」だけをえらび、
   お客様のページでは、その一覧ページへのリンクを出します。
   （2026/09/08 ユーザー指示「エコキュートと給湯器、品番不要！
     エコキュートのみ選択、給湯器のみ、選択！」
     「外壁・屋根、窓、ドア、内装改装もね」）
   ------------------------------------------------------------ */
function ymkrf_works_catlink_groups() {
	/* 商品ページをえらぶのは、キッチン・お風呂・トイレ・洗面化粧台だけ。
	   のこりは「この工事に入っています」のチェック1つです。 */
	$pick = array( 'kitchen', 'bathroom', 'toilet', 'toilet-counter', 'lavatory', 'other' );
	$out  = array();
	foreach ( array_keys( ymkrf_works_prod_groups_master() ) as $g ) {
		if ( ! in_array( $g, $pick, true ) ) $out[] = $g;
	}
	return $out;
}
...
function ymkrf_works_catlinks( $post_id ) {
	...
	$out = array();
	foreach ( explode( ',', $v ) as $g ) {
		$g = trim( $g );
		if ( $g !== '' && in_array( $g, ymkrf_works_catlink_groups(), true ) ) $out[] = $g;
	}
	return $out;
}
...
/* ------------------------------------------------------------
   1-b2. 施工エリア（市町）のえらびかた
         （2026/09/23 ユーザー指示
           「施工エリアを全エリアから選択できるようにして。
             一番下に、その他のエリアをつくっておいて
             エリア登録するように促して」）

   ・並びは、お客様の声で使っている市町の表（ymkrf_voice_city_roman）
     と同じにします。これで施工事例・お客様の声・対応エリアページの
     3つが、いつも同じ市町名でそろいます。
   ・すでに入っているけれど表に無いもの（矢田町・吉田郡など）は
     「そのほか（登録ずみ）」にまとめて出します。直すときの手がかりです。
   ------------------------------------------------------------ */

/** 県の名前（親になるもの） */
function ymkrf_works_area_prefs() {
	return array( '石川県' => 'ishikawa', '福井県' => 'fukui-ken' );
}

/**
 * 県の分類（親）を用意します。無ければ作ります。
 * （2026/09/23 ユーザー指示「エリアの親カテゴリに、福井県、石川県を設定」）
 */
function ymkrf_works_area_pref_term( $pref ) {

	$prefs = ymkrf_works_area_prefs();
	if ( ! isset( $prefs[ $pref ] ) ) return 0;

	$t = get_term_by( 'name', $pref, 'ymkrf_works_area' );
	if ( $t && ! is_wp_error( $t ) ) return (int) $t->term_id;

	/* URL用の名前。市町とぶつかるときは、うしろに -pref を付けます */
	$slug = $prefs[ $pref ];
	if ( get_term_by( 'slug', $slug, 'ymkrf_works_area' ) ) $slug .= '-pref';

	$new = wp_insert_term( $pref, 'ymkrf_works_area', array( 'slug' => $slug, 'parent' => 0 ) );
	if ( is_wp_error( $new ) ) {
		$e = term_exists( $pref, 'ymkrf_works_area' );
		return $e ? (int) $e['term_id'] : 0;
	}
	return (int) $new['term_id'];
}

/* ------------------------------------------------------------
   エリアの整理（2026/09/23 ユーザー指示
     「不要であれば削除して、親カテゴリを各市町に設定しておいて
       名前は市町郡だけでOK」）

   取り込んだデータには、住所の書きかたのちがいがそのまま残っています。

     石川県七尾市 ／ 七尾市 ／ 石川県鹿島郡中能登町 ／ 野々市 ／ 金津 …
   ...
   ・名前は市町郡だけ（県名・郡名は付けません）
   ・同じ市町が2つあるときは、記事を1つにまとめてから、空のほうを消します
   ・スラッグ（URLの英字）も、地域ページ /area/nanao/ とそろえます
   ------------------------------------------------------------ */

/** 郡（ぐん）の県と、URL用の英字 */
function ymkrf_area_gun() {
	return array(
		'羽咋郡' => array( '石川県', 'hakui-gun' ),
		'鹿島郡' => array( '石川県', 'kashima-gun' ),
		'河北郡' => array( '石川県', 'kahoku-gun' ),
		'能美郡' => array( '石川県', 'nomi-gun' ),
		'鳳珠郡' => array( '石川県', 'fugeshi-gun' ),
		'吉田郡' => array( '福井県', 'yoshida-gun' ),
		'丹生郡' => array( '福井県', 'nyu-gun' ),
		'今立郡' => array( '福井県', 'imadate-gun' ),
		'南条郡' => array( '福井県', 'nanjo-gun' ),
		'三方郡' => array( '福井県', 'mikata-gun' ),
		'大飯郡' => array( '福井県', 'oi-gun' ),
	);
}

/**
 * 分類の名前を、正しい市町郡の名前になおします。
 *
 *   石川県七尾市           → 七尾市
 *   石川県鹿島郡中能登町   → 中能登町
 *   野々市                 → 野々市市
 *   金津                   → あわら市（むかしの町名）
 */
function ymkrf_area_fix_name( $name ) {

	$n = trim( (string) $name );

	/* 県名を取ります */
	$n = preg_replace( '/^(石川県|福井県|富山県)/u', '', $n );
	$n = trim( $n );

	/* 「◯◯郡△△町」は、町のほうにします */
	if ( preg_match( '/^.+?郡(.+?[町村])$/u', $n, $m ) ) $n = $m[1];
...
function ymkrf_area_fix_bits( $name ) {
	...
	$gun = ymkrf_area_gun();
	if ( isset( $gun[ $name ] ) ) {
		return array( $gun[ $name ][0], $gun[ $name ][1] );
	}

	$map  = function_exists( 'ymkrf_voice_city_roman' ) ? ymkrf_voice_city_roman() : array();
	$slug = isset( $map[ $name ] ) ? $map[ $name ] : '';
	$pref = function_exists( 'ymkrf_area_pref' ) ? ymkrf_area_pref( $name ) : '石川県';

	return array( $pref, $slug );
}
...
add_action( 'admin_init', function () {
	...
	$log = array( 'moved' => array(), 'renamed' => array(), 'deleted' => array(), 'kept' => array() );

	/* ① 県（親）を作ります */
	$pids = array();
	foreach ( ymkrf_works_area_prefs() as $pref => $slug ) {
		$pids[ $pref ] = ymkrf_works_area_pref_term( $pref );
	}
	...
	/* ② 正しい名前ごとに、残す分類を1つ決めます */
	$keep = array();   /* 正しい名前 => term_id */
	$jobs = array();   /* あとで消す分類 */

	foreach ( $terms as $t ) {

		if ( in_array( (int) $t->term_id, $pids, true ) ) continue;   /* 県そのもの */

		$right = ymkrf_area_fix_name( $t->name );
		if ( $right === '' ) continue;

		if ( ! isset( $keep[ $right ] ) ) {
			$keep[ $right ] = (int) $t->term_id;
		} else {
			$jobs[] = array( (int) $t->term_id, $t->name, $keep[ $right ], $right );
		}
	}

	/* ③ 重なっている分類の記事を、残すほうに移してから消します */
	$retitle = array();
	foreach ( $jobs as $j ) {
		list( $from, $fromname, $to, $toname ) = $j;

		$posts = get_objects_in_term( $from, 'ymkrf_works_area' );
		if ( $posts && ! is_wp_error( $posts ) ) {
			foreach ( $posts as $pid ) {
				wp_remove_object_terms( (int) $pid, $from, 'ymkrf_works_area' );
				wp_add_object_terms( (int) $pid, $to, 'ymkrf_works_area' );
				$retitle[] = (int) $pid;
			}
			if ( $posts ) $log['moved'][] = $fromname . ' → ' . $toname . '（' . count( $posts ) . '件）';
		}

		wp_delete_term( $from, 'ymkrf_works_area' );
		$log['deleted'][] = $fromname;
	}

	/* ④ 残した分類の、名前・スラッグ・親をそろえます */
	foreach ( $keep as $right => $tid ) {

		$t = get_term( $tid, 'ymkrf_works_area' );
		if ( ! $t || is_wp_error( $t ) ) continue;

		list( $pref, $slug ) = ymkrf_area_fix_bits( $right );

		$args = array();
		if ( $t->name !== $right ) $args['name'] = $right;

		if ( $slug !== '' && $t->slug !== $slug ) {
			$other = get_term_by( 'slug', $slug, 'ymkrf_works_area' );
			if ( ! $other || (int) $other->term_id === (int) $tid ) $args['slug'] = $slug;
		}

		$pid = isset( $pids[ $pref ] ) ? $pids[ $pref ] : 0;
		if ( $pid && (int) $t->parent !== $pid ) $args['parent'] = $pid;

		if ( $args ) {
			$r = wp_update_term( $tid, 'ymkrf_works_area', $args );
			if ( ! is_wp_error( $r ) && isset( $args['name'] ) ) {
				$log['renamed'][] = $t->name . ' → ' . $right;
				$ps = get_objects_in_term( $tid, 'ymkrf_works_area' );
				if ( $ps && ! is_wp_error( $ps ) ) foreach ( $ps as $pid2 ) $retitle[] = (int) $pid2;
			}
		}

		if ( $slug === '' ) $log['kept'][] = $right;
	}

	/* ⑤ 名前が変わった事例の題名を作りなおします */
	foreach ( array_unique( $retitle ) as $pid ) {
		if ( ! function_exists( 'ymkrf_works_auto_title' ) ) break;
		$auto = ymkrf_works_auto_title( $pid );
		update_post_meta( $pid, '_ymkrf_auto_title', $auto );
		if ( $auto !== '' && $auto !== get_post_field( 'post_title', $pid ) ) {
			wp_update_post( array( 'ID' => $pid, 'post_title' => $auto ) );
		}
	}
...
add_action( 'admin_notices', function () {
	...
	$log = get_option( 'ymkrf_area_tidy_log' );
	if ( ! is_array( $log ) ) return;
	?>
	<div class="notice notice-success is-dismissible">
	  <p><b>エリアの整理がおわりました</b></p>
	  <?php foreach ( array(
	      'moved'   => '記事を移した分',
	      'renamed' => '名前をなおした分',
	      'deleted' => '消した分類',
	      'kept'    => 'ローマ字の表に無いので、スラッグはそのままにした分',
	  ) as $k => $ttl ) :
	    if ( empty( $log[ $k ] ) ) continue; ?>
	    <p><b><?php echo esc_html( $ttl ); ?>（<?php echo count( $log[ $k ] ); ?>）</b><br>
	       <?php echo esc_html( implode( '　/　', $log[ $k ] ) ); ?></p>
	  <?php endforeach; ?>
	</div>
	<?php
	delete_option( 'ymkrf_area_tidy_log' );
} );


/** えらべる市町の一覧（県ごと） */
function ymkrf_works_area_choices() {
	...
	if ( ! is_wp_error( $terms ) ) {
		foreach ( (array) $terms as $t ) $byname[ $t->name ] = $t;
	}

	foreach ( array_keys( $map ) as $city ) {
		$pref = function_exists( 'ymkrf_area_pref' ) ? ymkrf_area_pref( $city ) : '石川県';
		if ( ! isset( $out[ $pref ] ) ) $pref = '石川県';
		$out[ $pref ][] = $city;
	}

	/* 表に無いのに使われている名前（あとで直していただくぶん）。
	   県そのもの（石川県・福井県）は、えらぶものではないので出しません。 */
	$prefs = ymkrf_works_area_prefs();
	foreach ( $byname as $name => $t ) {
		if ( isset( $map[ $name ] ) )   continue;
		if ( isset( $prefs[ $name ] ) ) continue;
		$out['そのほか（登録ずみ）'][] = $name;
	}
...
/**
 * 市町の名前から「エリア」の分類を用意します（無ければ新しく作ります）。
 * 戻り値は分類のID。作れなかったときは 0。
 */
function ymkrf_works_area_term( $city ) {

	$city = function_exists( 'ymkrf_area_clean' ) ? ymkrf_area_clean( $city ) : trim( (string) $city );
	if ( $city === '' ) return 0;

	$t = get_term_by( 'name', $city, 'ymkrf_works_area' );
	if ( $t && ! is_wp_error( $t ) ) return (int) $t->term_id;

	/* ローマ字の表にある市町は、URL用の名前もそろえて作ります */
	$args = array();
	$slug = function_exists( 'ymkrf_area_roman' ) ? ymkrf_area_roman( $city ) : '';
	if ( $slug !== '' && ! get_term_by( 'slug', $slug, 'ymkrf_works_area' ) ) {
		$args['slug'] = $slug;
	}

	/* 県（親）の下に入れます */
	$pref = function_exists( 'ymkrf_area_pref' ) ? ymkrf_area_pref( $city ) : '';
	$pid  = $pref ? ymkrf_works_area_pref_term( $pref ) : 0;
	if ( $pid ) $args['parent'] = $pid;

	$new = wp_insert_term( $city, 'ymkrf_works_area', $args );
	if ( is_wp_error( $new ) ) {
		$exists = term_exists( $city, 'ymkrf_works_area' );
		return $exists ? (int) $exists['term_id'] : 0;
	}
	return (int) $new['term_id'];
}
...
add_action( 'wp_ajax_ymkrf_area_reload', function () {
	...
	check_ajax_referer( 'ymkrf_area_reload' );
	if ( ! current_user_can( 'edit_ymkrf_works' ) && ! current_user_can( 'edit_posts' ) ) {
		wp_send_json_error();
	}

	$out = array();
	foreach ( ymkrf_works_area_choices() as $pref => $cities ) {
		$list = array();
		foreach ( $cities as $city ) $list[] = array( 'v' => $city, 't' => $city );
		$out[ $pref ] = $list;
	}
...
/* ------------------------------------------------------------
   エリアの一覧を見やすくします
   （2026/09/23 ユーザー指示
     「エリアの一覧、説明も不要。対応店舗だけ一覧で欲しいかも。
       並び順、店舗ごとだと見やすいが」）

   ・「説明」の列と入力欄は出しません
   ・かわりに「対応店舗」の列を出します（店舗の担当エリアから引いています）
   ・上に店舗のボタンを並べて、その店の担当エリアだけを見られるようにします
     （並べかえにしないのは、石川県・福井県の親子の並びをくずさないためです）
   ------------------------------------------------------------ */

/** 市町 → 担当しているお店の名前 */
function ymkrf_area_shopmap() {
	...
	foreach ( ymkrf_shops() as $s ) {
		...
		foreach ( (array) ( isset( $s['areas'] ) ? $s['areas'] : array() ) as $a ) {

			/* 「志賀町（一部）」のような書きかたも受けます */
			$part = ( strpos( $a, '（一部）' ) !== false );
			$city = trim( str_replace( '（一部）', '', $a ) );
			if ( function_exists( 'ymkrf_area_clean' ) ) $city = ymkrf_area_clean( $city );
			if ( $city === '' ) continue;

			if ( ! isset( $out[ $city ] ) ) $out[ $city ] = array();
			$out[ $city ][] = array(
...
/* 列の出し入れ */
add_filter( 'manage_edit-ymkrf_works_area_columns', function ( $cols ) {
	unset( $cols['description'] );
	$new = array();
	foreach ( $cols as $k => $v ) {
		$new[ $k ] = $v;
		if ( $k === 'name' ) $new['ymkrf_shop'] = '対応店舗';
	}
	if ( ! isset( $new['ymkrf_shop'] ) ) $new['ymkrf_shop'] = '対応店舗';
	return $new;
} );

add_filter( 'manage_ymkrf_works_area_custom_column', function ( $out, $col, $term_id ) {

	if ( $col !== 'ymkrf_shop' ) return $out;

	$t = get_term( $term_id, 'ymkrf_works_area' );
	if ( ! $t || is_wp_error( $t ) ) return $out;

	/* 県（親）には出しません */
	$prefs = ymkrf_works_area_prefs();
	if ( isset( $prefs[ $t->name ] ) ) return '—';

	$map = ymkrf_area_shopmap();
	if ( empty( $map[ $t->name ] ) ) {
		return '<span style="color:#b32d2e">店舗の担当エリアに未登録</span>';
	}

	$names = array();
	foreach ( $map[ $t->name ] as $s ) $names[] = esc_html( $s['name'] );
...
add_filter( 'get_terms_args', function ( $args, $taxonomies ) {
	...
	if ( empty( $_GET['ymkrf_shop'] ) ) return $args;
	if ( ! isset( $args['page'] ) ) return $args;                 /* 一覧の読み込みだけ */
	if ( (array) $taxonomies !== array( 'ymkrf_works_area' ) ) return $args;

	$want = sanitize_key( $_GET['ymkrf_shop'] );
	$map  = ymkrf_area_shopmap();

	$cities = array();
	foreach ( $map as $city => $shops ) {
		foreach ( $shops as $one ) {
			if ( $one['slug'] === $want ) { $cities[] = $city; break; }
		}
	}

	$ids = array();
	foreach ( ymkrf_works_area_prefs() as $pref => $x ) {
		$pt = get_term_by( 'name', $pref, 'ymkrf_works_area' );
		if ( $pt && ! is_wp_error( $pt ) ) $ids[] = (int) $pt->term_id;
	}

	if ( $want === '_none' ) {
		/* どの店舗の担当にも入っていないエリア */
		$all = get_terms( array( 'taxonomy' => 'ymkrf_works_area', 'hide_empty' => false ) );
		if ( $all && ! is_wp_error( $all ) ) {
			$prefs = ymkrf_works_area_prefs();
			foreach ( $all as $t ) {
				if ( isset( $prefs[ $t->name ] ) ) continue;
				if ( empty( $map[ $t->name ] ) ) $ids[] = (int) $t->term_id;
			}
		}
	} else {
...
add_action( 'add_meta_boxes', function () {
	...
	/* 「エリア」も施工データの中に入れました（2026/09/23 ユーザー指示
	   「施工エリアをお客様の前に、全エリアから選択できるように」）。
	   右側の箱を残すと二重に入って、矢田町のような町名が混ざるもとになります。 */
	remove_meta_box( 'ymkrf_works_areadiv', 'ymkrf_works', 'side' );
...
function ymkrf_works_metabox( $post ) {
	...
	$selcl = ymkrf_works_catlinks( $post->ID );
	...
	<div class="ymkrf-works">
	  <table class="form-table ymkrf-works__table">
	    ...
	    <?php /* 施工エリア（2026/09/23 追加）。
	             ここでえらんだ市町が、そのまま題名とURLの地域になります。 */ ?>
	    <tr>
	      <th>施工エリア <span class="ymkrf-need">必須</span></th>
	      <td>
	        <?php $areacur = ymkrf_works_area_name( $post->ID ); ?>
	        <select name="_ymkrf_areapick" id="ymkrf-area-sel">
	          <option value="">（えらんでください）</option>
	          <?php
	          $hit = ( $areacur === '' );
	          foreach ( ymkrf_works_area_choices() as $pref => $cities ) : ?>
	            <optgroup label="<?php echo esc_attr( $pref ); ?>">
	              <?php foreach ( $cities as $city ) :
	                $sel = ( $city === $areacur );
	                if ( $sel ) $hit = true; ?>
	                <option value="<?php echo esc_attr( $city ); ?>" <?php selected( $sel ); ?>>
	                  <?php echo esc_html( $city ); ?></option>
	              <?php endforeach; ?>
	            </optgroup>
	          <?php endforeach; ?>
	          <option value="__other__" <?php selected( ! $hit ); ?>>その他のエリア（一覧にないとき）</option>
	        </select>

	        <button type="button" class="button" id="ymkrf-area-load">登録したエリアを読みこむ</button>
	        <span id="ymkrf-area-done" class="ymkrf-area__done" style="display:none"></span>

	        <p class="description">
	          お客様のお住まいの<b>市・町</b>をえらんでください。<br>
	          題名（○○市 Y様）と、地域ごとのページ（/area/○○/）は、ここから作られます。
	        </p>
	        <p class="description ymkrf-area__note" id="ymkrf-area-note"
	           style="<?php echo $hit ? 'display:none' : ''; ?>">
	          <b>「その他のエリア」をえらぶと、エリアの登録画面が別の画面で開きます。</b><br>
	          そちらで市町を登録したら、この画面にもどって
	          <b>［登録したエリアを読みこむ］</b>を押してください。
	          いま入力中の内容はそのままで、一覧に追加されます。<br>
	          町名・字（あざ）ではなく、かならず<b>市・町の名前</b>で登録してください。
	          （例：○ 七尾市　／　× 矢田町）<br>
	          <a href="<?php echo esc_url( admin_url( 'edit-tags.php?taxonomy=ymkrf_works_area&post_type=ymkrf_works' ) ); ?>"
	             target="_blank" rel="noopener" id="ymkrf-area-open">エリアの登録画面をひらく</a>
	        </p>
	      </td>
	    </tr>
	    ...
	    <tr>
	      ...
	      <td>
            ...
            <?php $iscl0 = in_array( $gk, ymkrf_works_catlink_groups(), true ); ?>
	            <div class="ymkrf-works__pgroup<?php echo $iscl0 ? ' ymkrf-works__pgroup--one' : ''; ?>">
	              <?php $iscl = in_array( $gk, ymkrf_works_catlink_groups(), true ); ?>
	              <p class="ymkrf-works__pttl">
	                <?php if ( $iscl ) : ?>
	                  <?php /* 品番はえらばないので、見出しの左にチェックを置きます
	                           （2026/09/08 ユーザー指示） */ ?>
	                  <label class="ymkrf-works__pttlchk">
	                    <input type="checkbox" name="_ymkrf_catlink[]"
	                           value="<?php echo esc_attr( $gk ); ?>"
	                      <?php checked( in_array( $gk, $selcl, true ) ); ?>>
	              ...
	              <?php if ( $iscl ) : ?>
	                <?php /* もし前に品番をえらんであったら、そのまま残します */ ?>
	                <?php foreach ( $g['items'] as $pr ) :
	                  if ( ! in_array( (int) $pr->ID, $sel, true ) ) continue; ?>
	                  <input type="hidden" name="_ymkrf_products[]" value="<?php echo (int) $pr->ID; ?>">
	                <?php endforeach; ?>
	              <?php elseif ( $g['items'] ) : ?>
	                <div class="ymkrf-works__prods">
	                  <?php foreach ( $g['items'] as $pr ) : ?>
	                    <label>
	                      ...
	                      <span><?php
	                        /* 「手洗い付きカウンタートイレ」のまとまりでは、
	                           商品名のうしろの【手洗いカウンター付】は出しません。
	                           見出しに書いてあるので、二重になるためです。
	                           （2026/09/08 ユーザー指示）
	                           ※商品そのものの名前は変えていません。商品ページでは、
	                             手洗いカウンターの有り無しを見分けるのに使っています。 */
	                        $plabel = (string) get_the_title( $pr );
	                        if ( $gk === 'toilet-counter' ) {
	                          $plabel = trim( preg_replace( '/【[^】]*(手洗い|カウンター)[^】]*】/u', '', $plabel ) );
	                        }
	                        echo esc_html( $plabel );
	                      ?></span>
	                    </label>
	                  <?php endforeach; ?>
	                </div>
	              ...
	              <?php if ( $gk === 'other' ) : ?>
	                <?php
	                /* 商品ページが無いものは、ここに書きます。1行に1商品。
	                   左が「旧パック商品」のチェック、右がこのサイトの中の
	                   関連ページ（エコキュートや給湯器の一覧など）のURLです。
	                   （2026/09/08 ユーザー指示「一行にしたい。その下に、
	                     このサイトの関連ぺージのURLを入力する欄作って」） */
	                $ptlines = array_values( array_filter( array_map( 'trim',
	                  preg_split( '/\R/u', (string) $get( '_ymkrf_product_text' ) ) ),
	                  function ( $v ) { return $v !== ''; } ) );
	                $ptpack  = array_values( array_filter( array_map( 'trim',
	                  preg_split( '/\R/u', (string) $get( '_ymkrf_ptext_pack' ) ) ),
	                  function ( $v ) { return $v !== ''; } ) );

	                /* URLは「商品名|URL」の形でしまってあります */
	                $pturl = array();
	                foreach ( preg_split( '/\R/u', (string) $get( '_ymkrf_ptext_url' ) ) as $u ) {
	                  $u = trim( $u );
	                ...
	                $ptrows = max( 5, count( $ptlines ) + 1 );
	                ?>
	                <div class="ymkrf-works__ptext">
	                  <p class="ymkrf-works__ptextttl">関連ページが無い商品</p>

	                  <div class="ymkrf-works__ptrows">
	                    <div class="ymkrf-works__pthead">
	                      <span>旧パック<br>✓欄</span><span>商品名</span><span>関連ページのURL（任意）</span>
	                    </div>
	                    <?php for ( $i = 0; $i < $ptrows; $i++ ) :
	                      $pv = isset( $ptlines[ $i ] ) ? $ptlines[ $i ] : '';
	                      $uv = ( $pv !== '' && isset( $pturl[ $pv ] ) ) ? $pturl[ $pv ] : ''; ?>
	                      <div class="ymkrf-works__ptrow">
	                        <label class="ymkrf-works__ptchk">
	                          <input type="checkbox" name="_ymkrf_ptext_pack[<?php echo $i; ?>]" value="1"
	                            <?php checked( $pv !== '' && in_array( $pv, $ptpack, true ) ); ?>>
	                          <span class="screen-reader-text">旧パック商品</span>
	                        </label>
	                        <?php /* 何行目かを入力例に書いておくと、
	                                 「1行に1商品」がひと目で伝わります（2026/09/08） */ ?>
	                        <input type="text" name="_ymkrf_ptext[<?php echo $i; ?>]"
	                               value="<?php echo esc_attr( $pv ); ?>"
	                               placeholder="<?php echo esc_attr(
	                                 ( $i + 1 ) . 'つめの商品'
	                                 . ( $i === 0 ? '（例：トクラス　Bbプラス）' : '' ) ); ?>">
	                        <input type="text" name="_ymkrf_ptext_url[<?php echo $i; ?>]"
	                               value="<?php echo esc_attr( $uv ); ?>"
	                               placeholder="<?php echo $i === 0 ? '例：/products/ecocute/' : ''; ?>">
	                      </div>
	                    <?php endfor; ?>
	                  </div>

	                  <p class="ymkrf-works__oldpacknote">
	                    <b>旧パック</b>にチェックを入れた商品には、ページで
	                    「※こちらはヤマキシ旧パック商品となります」と出ます。<br>
	                    <b>関連ページのURL</b>を入れると、その商品がリンクになります。
	                    このサイトの中のページにしてください
	                    （例：<code>/products/ecocute/</code>　<code>/products/boiler/</code>）。
	                  </p>
...
add_action( 'admin_head', function () {
	...
	echo '<style>
	  ...
	  /* 使った商品 */
	  /* 中で縦にスクロールさせず、そのまま全部出します（2026/09/08 ユーザー指示） */
	  .ymkrf-works__pgroups{
	    max-width:1240px;border:1px solid #dcdcde;border-radius:8px;background:#fff}
	  ...
	  /* 品番をえらばないまとまりは、チェック1つだけなので、
	     枠を付けず、高さもつめます（2026/09/08 ユーザー指示） */
	  .ymkrf-works__pgroup--one{padding:3px 14px;border-bottom:0}
	  .ymkrf-works__pgroup--one .ymkrf-works__pttl{margin:0}
	  .ymkrf-works__pgroup--one + .ymkrf-works__pgroup:not(.ymkrf-works__pgroup--one){
	    border-top:1px solid #f0f0f1;padding-top:12px}
	  .ymkrf-works__pttlchk{
	    display:inline-flex;align-items:center;gap:8px;margin:0;cursor:pointer}
	  .ymkrf-works__pttlchk input{margin:0}
	  .ymkrf-works__pttlchk span{font-size:13px;font-weight:700;color:#1d2327}
	  .ymkrf-works__pttlchk:hover span{color:#fe3301}
	  .ymkrf-works__pnum{
	  ...
	  /* チェックと商品名は、かならず横に並べます。
	     名前が長いときは、名前のほうだけで折り返します。 */
	  ...
	  .ymkrf-works__prods label > span{min-width:0;word-break:break-word}
	  .ymkrf-works__prods input{margin:2px 0 0}

	  /* ページが無い商品の入力欄（1行＝旧パック／商品名／関連ページのURL） */
	  .ymkrf-works__pempty{margin:0;font-size:12px;color:#8c8f94;line-height:1.7}
	  .ymkrf-works__ptextttl{margin:0 0 8px;font-size:12px;font-weight:700;color:#1d2327}
	  .ymkrf-works__ptextttl span{display:inline-block;margin-left:8px;font-weight:400;color:#646970}
	  .ymkrf-works__ptrows{display:grid;gap:6px;max-width:840px}
	  .ymkrf-works__pthead,
	  .ymkrf-works__ptrow{
	    display:grid;grid-template-columns:66px minmax(0,1fr) minmax(0,1fr);
	    gap:8px;align-items:center}
	  .ymkrf-works__pthead{font-size:11px;font-weight:700;color:#646970;line-height:1.4}
	  .ymkrf-works__pthead span:first-child{text-align:center}
	  .ymkrf-works__ptrow input[type=text]{width:100%;margin:0;height:30px}
	  /* 枠は付けず、商品名の欄と同じ高さのまん中にチェックを置きます
	     （2026/09/08 ユーザー指示） */
	  .ymkrf-works__ptchk{
	    display:flex !important;align-items:center;justify-content:center;margin:0 !important;
	    height:30px}
	  .ymkrf-works__ptchk input{margin:0}
	  .ymkrf-works__oldpacknote{
	    margin:10px 0 0;font-size:12px;color:#646970;line-height:1.8}
	  .ymkrf-works__oldpacknote code{font-size:12px}
	  .ymkrf-works__oldpack{
	    display:grid !important;grid-template-columns:22px 1fr;align-items:center;
	    margin:10px 0 0 !important;padding:6px 10px;max-width:840px;
	  ...
	  @media (max-width:860px){
	    .ymkrf-works__pthead{display:none}
	    .ymkrf-works__ptrow{grid-template-columns:66px minmax(0,1fr)}
	    .ymkrf-works__ptrow input[type=text]:last-child{grid-column:2}
	  }
	  ...
	  .ymkrf-area__done{
	    ...
	    background:#e6f4ea;color:#1e7b34;font-size:12px;font-weight:700}
	  .ymkrf-area__note{
	    margin-top:8px;padding:10px 12px;border-left:4px solid #fe3301;
	    background:#fff6f3;border-radius:0 4px 4px 0}
...
/* 「抜粋」の欄に、この現場ならではの説明を出します。
   WordPressのそのままの文だと、何を書く欄か分かりにくいためです。
   （2026/09/08 ユーザー「抜粋って何を入れるの？」） */
add_action( 'admin_footer-post.php', function () {
	...
	jQuery(function ($) {
		...
		$box.find('.inside p').html(
			...
			'この現場ならではの見どころを、<b>40〜80文字</b>くらいで書いてください。<br>' +
			'空のままでも大丈夫です。そのときは本文の頭から90文字が自動で使われます。'
		);
		$box.find('#excerpt').attr('placeholder',
			'例：築24年のキッチンを対面のI型に。袖壁を撤去して、キッチンからリビングまで見渡せるようにしました。');
	});
	</script>
	<?php
} );
...
add_action( 'admin_footer', function () {
	...
	jQuery(function ($) {
		...
		/* 施工エリア ─ 「その他のエリア」をえらんだときだけ、入力欄を出します
		   （2026/09/23 追加） */
		var $areaSel = $('#ymkrf-area-sel');
		if ($areaSel.length) {

			var areaWin  = null;
			var areaBack = $areaSel.val();   /* 「その他」の前にえらんでいたもの */
			var areaUrl  = $('#ymkrf-area-open').attr('href');

			/* 「その他のエリア」→ 登録画面を別の画面で開きます
			   （2026/09/23 ユーザー指示） */
			$areaSel.on('change', function () {
				var other = ($(this).val() === '__other__');
				$('#ymkrf-area-note').toggle(other);
				if (!other) { areaBack = $(this).val(); return; }

				try { areaWin = window.open(areaUrl, 'ymkrfArea'); } catch (e) { areaWin = null; }
				if (areaWin) { try { areaWin.focus(); } catch (e) {} }
				else {
					window.alert('別の画面が開けませんでした。'
						+ '下の「エリアの登録画面をひらく」を押してください。');
				}
			});

			/* 登録画面で足した市町を、この画面の一覧に入れなおします。
			   入力中の内容は、そのまま残ります。 */
			$('#ymkrf-area-load').on('click', function () {

				var $b = $(this).prop('disabled', true).text('読みこみ中…');
				var before = [];
				$areaSel.find('option').each(function () { before.push(this.value); });

				$.post(ajaxurl, {
					action: 'ymkrf_area_reload',
					_wpnonce: <?php echo wp_json_encode( wp_create_nonce( 'ymkrf_area_reload' ) ); ?>
				}).done(function (res) {

					if (!res || !res.success) { window.alert('読みこめませんでした。'); return; }

					var keep = $areaSel.val();
					var html = '<option value="">（えらんでください）</option>';
					var added = null;
					var esc = function (s) { return $('<div>').text(String(s)).html(); };

					$.each(res.data.groups, function (pref, cities) {
						html += '<optgroup label="' + esc(pref) + '">';
						$.each(cities, function (i, c) {
							html += '<option value="' + esc(c.v) + '">' + esc(c.t) + '</option>';
							if (before.indexOf(c.v) === -1) added = c.v;
						});
						html += '</optgroup>';
					});
					html += '<option value="__other__">その他のエリア（一覧にないとき）</option>';

					$areaSel.html(html);

					/* 新しく増えた市町があれば、それをえらんでおきます */
					var pick = added || (keep === '__other__' ? areaBack : keep) || '';
					$areaSel.val(pick);
					if ($areaSel.val() !== pick) $areaSel.val('');

					$('#ymkrf-area-note').toggle($areaSel.val() === '__other__');
					$('#ymkrf-area-done')
						.text(added ? '「' + added + '」をえらびました' : '一覧を新しくしました')
						.show().delay(4000).fadeOut();

				}).fail(function () {
					window.alert('読みこめませんでした。');
				}).always(function () {
					$b.prop('disabled', false).text('登録したエリアを読みこむ');
				});
			});
		}
...
add_action( 'save_post_ymkrf_works', function ( $post_id ) {
	...
	/* ページが無い商品（1行に1商品）と、旧パックの印、関連ページのURL。
	   空の行はとばして、上から詰めて入れなおします。（2026/09/08） */
	if ( isset( $_POST['_ymkrf_ptext'] ) ) {
		$ptin  = (array) wp_unslash( $_POST['_ymkrf_ptext'] );
		$ptck  = isset( $_POST['_ymkrf_ptext_pack'] ) ? (array) $_POST['_ymkrf_ptext_pack'] : array();
		$ptu   = isset( $_POST['_ymkrf_ptext_url'] ) ? (array) wp_unslash( $_POST['_ymkrf_ptext_url'] ) : array();
		$lines = array();
		$packs = array();
		$urls  = array();
		foreach ( $ptin as $i => $v ) {
			...
			if ( $v === '' ) continue;
			if ( in_array( $v, $lines, true ) ) continue;
			$lines[] = $v;
			if ( ! empty( $ptck[ $i ] ) ) $packs[] = $v;

			$u = isset( $ptu[ $i ] ) ? trim( (string) $ptu[ $i ] ) : '';
			if ( $u !== '' ) {
				/* 「/products/ecocute/」のような、サイトの中の書きかたも受けます */
				if ( strpos( $u, '/' ) !== 0 ) $u = esc_url_raw( $u );
				if ( $u !== '' ) $urls[] = $v . '|' . $u;
			}
		}
		update_post_meta( $post_id, '_ymkrf_product_text', implode( "\n", $lines ) );
		if ( $packs ) update_post_meta( $post_id, '_ymkrf_ptext_pack', implode( "\n", $packs ) );
		else          delete_post_meta( $post_id, '_ymkrf_ptext_pack' );
		if ( $urls )  update_post_meta( $post_id, '_ymkrf_ptext_url', implode( "\n", $urls ) );
		else          delete_post_meta( $post_id, '_ymkrf_ptext_url' );
	}

	/* えらんだ商品ページ（キッチン・お風呂・トイレ・洗面化粧台） */
	...
	/* 品番をえらばないまとまり（給湯器・エコキュートほか）。
	   ページでは一覧ページへのリンクになります。（2026/09/08） */
	$cl = isset( $_POST['_ymkrf_catlink'] ) ? (array) wp_unslash( $_POST['_ymkrf_catlink'] ) : array();
	$cl = array_values( array_intersect( ymkrf_works_catlink_groups(),
	        array_map( 'sanitize_text_field', $cl ) ) );
	if ( $cl ) update_post_meta( $post_id, '_ymkrf_catlink', implode( ',', $cl ) );
	else       delete_post_meta( $post_id, '_ymkrf_catlink' );
	...
	/* 施工エリア（市町）。右側の箱は消してあるので、ここで入れます。
	   「その他のエリア」をえらんだときは、打っていただいた市町名を
	   「エリア」に新しく登録してから結びつけます。
	   （2026/09/23 ユーザー指示） */
	if ( isset( $_POST['_ymkrf_areapick'] )
	  && ( current_user_can( 'assign_terms', 'ymkrf_works_area' )
	    || current_user_can( 'edit_ymkrf_works' ) ) ) {

		$pick = sanitize_text_field( wp_unslash( $_POST['_ymkrf_areapick'] ) );

		/* 「その他のエリア」のまま保存されたときは、いまのエリアをそのままにします
		   （登録画面を開いただけで消えてしまわないように） */
		if ( $pick === '__other__' ) $pick = ymkrf_works_area_name( $post_id );

		if ( $pick === '' ) {
			wp_set_object_terms( $post_id, array(), 'ymkrf_works_area', false );
		} else {
			$tid = ymkrf_works_area_term( $pick );
			if ( $tid ) wp_set_object_terms( $post_id, array( $tid ), 'ymkrf_works_area', false );
		}
	}
}, 10 );
...
function ymkrf_works_area_name( $post_id ) {
	...
	if ( $ts && ! is_wp_error( $ts ) ) {
		...
		/* 県（石川県・福井県）は親なので、題名には使いません。
		   市町のほうを先に探します。（2026/09/23） */
		$prefs = function_exists( 'ymkrf_works_area_prefs' ) ? ymkrf_works_area_prefs() : array();
		foreach ( $ts as $t ) {
			if ( isset( $prefs[ $t->name ] ) ) continue;
			return $t->name;
		}
...
/* 抜粋（＝検索結果に出る説明文）が空のときは、
   本文やおこなった工事から自動で入れておきます。（2026/09/08） */
add_action( 'save_post_ymkrf_works', function ( $post_id ) {
