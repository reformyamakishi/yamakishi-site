...
/* ============================================================
   ...
   ============================================================ */
/**
 * 「商品仕様」の文字を、1行に1商品の形にととのえます。
 *
 * いまのサイトからもらってくると、
 *   リクシル リシェル この商品の情報を見る ミーレ 食洗器 この商品の情報を見る
 * のように、リンクの文字をはさんで数珠つなぎになっています。
 * これを
 *   リクシル リシェル
 *   ミーレ 食洗器
 * にします。すでに改行で分かれているときは、そのまま活かします。
 */
function ymkrf_works_spec_lines( $raw ) {

	$t = (string) $raw;

	/* リンクの文字は、そこで切るための目印にします */
	$t = preg_replace( '/(この商品(の情報)?を(見る|みる)|商品ページを見る|くわしく見る)/u', "\n", $t );

	/* 「・」や「／」でつないであるだけのときも、そこで切ります */
	$t = str_replace( array( '　', "\r\n", "\r" ), array( ' ', "\n", "\n" ), $t );

	$out = array();
	foreach ( preg_split( '/\n/u', $t ) as $line ) {
		$line = trim( preg_replace( '/[ \t]+/u', ' ', $line ) );
		/* 前後の「・」「／」などを取ります。trim( $line, '・…' ) は
		   バイト単位で削ってしまい、日本語が壊れるので使いません。 */
		$line = preg_replace( '/^[・\/／、,\s]+|[・\/／、,\s]+$/u', '', $line );
		if ( $line === '' ) continue;
		if ( in_array( $line, $out, true ) ) continue;   // 同じものは1つに
		$out[] = $line;
	}
	return implode( "\n", $out );
}
...
function ymkrf_works_import_parse( $html, $url ) {
	...
	/* 商品仕様は本文に入れず、「使った商品」の欄に入れます。
	   いまのサイトでは、商品名のうしろに「この商品の情報を見る」という
	   リンクの文字が付いてくるので、そこで1商品ずつに切り分けます。
	   1行に1商品でしまうと、お客様のページで1つずつ並べられます。 */
	$spec = ymkrf_works_spec_lines( $get( '商品仕様' ) );
...
function ymkrf_works_import_city( $addr ) {
	...
	/* ここから下は、いまのサイトの住所がバラバラなのをそろえます。
	   「石川県七尾市」「加賀市片山津温泉」「石川県鹿島郡中能登町」などを、
	   すべて市町村の単位（七尾市／加賀市／中能登町）にします。 */

	/* 1. 県名を落とします */
	$a = preg_replace( '/^(石川県|福井県|富山県)/u', '', $a );

	/* 2. 郡の名前だけのときは、その郡にひとつしかない町にします。
	      羽咋郡・鳳珠郡は町が2つ以上あるので決められません（空にします）。 */
	if ( preg_match( '/^鹿島郡$/u', $a ) )        return '中能登町';
	if ( preg_match( '/^(能美郡|能美群)$/u', $a ) ) return '川北町';
	if ( preg_match( '/^丹生郡$/u', $a ) )        return '越前町';
	if ( preg_match( '/^吉田郡$/u', $a ) )        return '永平寺町';
	if ( preg_match( '/^(羽咋郡|鳳珠郡)$/u', $a ) ) return '';

	/* 3. 郡の名前は落として、そのうしろの町を見ます */
	$a = preg_replace( '/^(鹿島郡|能美郡|能美群|羽咋郡|鳳珠郡|丹生郡|吉田郡)/u', '', $a );

	/* 4. 「◯◯市」で切ります（加賀市片山津温泉 → 加賀市） */
	if ( preg_match( '/^(.+?市)/u', $a, $m ) ) {
		return ( $m[1] === '野々市' ) ? '野々市市' : $m[1];
	}

	/* 5. 町は、いまお付き合いのある町の名前で切ります */
	if ( preg_match( '/^(中能登町|志賀町|宝達志水町|川北町|津幡町|内灘町|穴水町|能登町|越前町|池田町|永平寺町)/u', $a, $m ) ) {
		return $m[1];
	}
	if ( preg_match( '/^(.+?[町村])/u', $a, $m ) ) return $m[1];

	/* 6. むかしの言いかた */
	if ( preg_match( '/^野々市/u', $a ) ) return '野々市市';
	if ( preg_match( '/^金津/u',   $a ) ) return 'あわら市';
