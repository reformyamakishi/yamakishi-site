 ...
 * ★2026/09/04 ユーザーから地図データ（PNG）をいただき、差し替えました。
 *
 * ── 作り方について ────────────────────────────────
 * 地図そのものは、いただいた画像（assets/img/shops/area-map.png）です。
 * その上に、店名のふだ・ピン・引き出し線をSVGで重ねています。
 *   ・店名は「文字」なので、押せますし、あとから直せます
 *   ・SVGなので、画面の大きさに合わせてぴったり拡大・縮小します
 *
 * ── ピンの位置について ────────────────────────────
 * お店の緯度・経度から計算しています。
 * 地図の3つの地点（能登半島の先・石川の西のはし・福井の西のはし）を
 * 手がかりにして、緯度経度から画像の座標に変換しました。
 * 11店すべて、正しい県の上にのっていることを確認ずみです。
 *
 * ── スマートフォンでの見せ方 ────────────────────────
 * 画面のはばいっぱいに出します（2026/09/04 ユーザー指定。横スクロールなし）。
 * 小さくなるぶん、店名の文字はCSSで大きくしています（flyer.css）。
 * 下の店名カードからでも、同じようにえらべます。
 *
 * ── 位置を直したいとき ────────────────────────────
 * $pins の数字を変えます（地図画像の座標。画像は 1350 × 784 です）。
 *   x, y   … ピンの場所
 *   lx, ly … 店名のふだの場所
...
function ymkrf_flyer_map( $shops, $sel, $map ) {
	...
	/* 画面に出す範囲（viewBox）。
	   まわりの余白と、福井の西のはし（若狭のあたり）を切って、
	   そのぶん地図と店名を大きく見せています（2026/09/04 ユーザー指定）。 */

	$pins = array(
		'tazuruhama'      => array( 'x' => 979, 'y' => 192, 'lx' => 830, 'ly' =>  95, 'side' => 'l' ),
		'hakui'           => array( 'x' => 903, 'y' => 232, 'lx' => 775, 'ly' => 175, 'side' => 'l' ),
		'kawakita'        => array( 'x' => 756, 'y' => 340, 'lx' => 655, 'ly' => 250, 'side' => 'l' ),
		'shinkaga'        => array( 'x' => 647, 'y' => 382, 'lx' => 575, 'ly' => 330, 'side' => 'l' ),
		'kanadu'          => array( 'x' => 566, 'y' => 414, 'lx' => 475, 'ly' => 405, 'side' => 'l' ),
		'asahi'           => array( 'x' => 538, 'y' => 471, 'lx' => 470, 'ly' => 480, 'side' => 'l' ),
		'higashikanazawa' => array( 'x' => 858, 'y' => 299, 'lx' => 1080, 'ly' => 210, 'side' => 'r' ),
		'tagami'          => array( 'x' => 877, 'y' => 303, 'lx' => 1080, 'ly' => 285, 'side' => 'r' ),
		'nonoichi'        => array( 'x' => 823, 'y' => 316, 'lx' => 1060, 'ly' => 360, 'side' => 'r' ),
		'komathu'         => array( 'x' => 711, 'y' => 355, 'lx' => 1010, 'ly' => 440, 'side' => 'r' ),
		'kahahothu'       => array( 'x' => 597, 'y' => 435, 'lx' => 900, 'ly' => 545, 'side' => 'r' ),
	);
	?>
	<div class="p-fmap">
	  <svg class="p-fmap__svg" viewBox="328 30 970 700"
	    ...
	    <image href="<?php echo esc_url( $dir . '/assets/img/shops/area-map.png' ); ?>"
	           x="0" y="0" width="<?php echo (int) $mw; ?>" height="<?php echo (int) $mh; ?>"></image>
	    ...
	    ?>
	      <a class="p-flyer__pick p-fmap__pin<?php echo $on ? ' is-on' : ''; ?><?php echo $soon ? ' is-soon' : ''; ?>"
	        ...
	        <circle class="p-fmap__ring" cx="<?php echo esc_attr( $p['x'] ); ?>"
	                cy="<?php echo esc_attr( $p['y'] ); ?>" r="13"></circle>
	        <circle class="p-fmap__dot" cx="<?php echo esc_attr( $p['x'] ); ?>"
	                cy="<?php echo esc_attr( $p['y'] ); ?>" r="6"></circle>
	        <rect class="p-fmap__box" x="<?php echo esc_attr( $bx ); ?>" y="<?php echo esc_attr( $by ); ?>"
	              width="<?php echo esc_attr( $bw ); ?>" height="52" rx="26"></rect>
	        <text class="p-fmap__label" x="<?php echo esc_attr( $bx + $bw / 2 ); ?>"
	              y="<?php echo esc_attr( $p['ly'] ); ?>"><?php echo esc_html( $sp['name'] ); ?></text>
	      </a>
	    <?php endforeach; ?>
	  </svg>
	</div>
