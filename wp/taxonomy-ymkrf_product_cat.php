...
if ( $slug === 'boiler' ) {
	...
	/* 給湯器の並べ方（2026/09/01 ユーザー指示）
	     ...
	     お客様が最初に決まっているのは「うちはガスか灯油か」なので、
	     そこで大きく分けると、半分は見なくて済みます。
	     エコキュートのページと同じ考え方です。
	   ...
	   ★どの種類かは、その商品の「キャッチコピー」で見分けています。
	     ...
	     上の2つが無くて「石油」→ 石油給湯器
	     上の3つが無くて「ガス」→ ガス給湯器
	     どれも入っていない商品は、見出しなしで最後に並びます。

	   ★メーカーの説明は、一覧のいちばん上に
	     「取り扱いメーカー」としてまとめて出します。
	     文章はダッシュボードの「商品 → メーカー」の「説明」欄で直せます。
	   ★並べる順は、下の $fuelgroups を書きかえてください。 */

	$maker_order = array( 'noritz', 'rinnai' );

	/* 大分類（燃料）→ 小見出し（種類）。安い順に並べています */
	$fuelgroups = array(
		'gas' => array(
			'ttl'  => 'ガス給湯器',
			'sub'  => '都市ガス・プロパンガスをお使いのお宅用です。家の外の壁に掛けて取り付けます。',
			'subs' => array(
				'gas'     => 'ガス給湯器',
				'ecojaws' => 'エコジョーズ（高効率ガス給湯器）',
			),
		),
		'oil' => array(
			'ttl'  => '石油給湯器',
			'sub'  => '灯油をお使いのお宅用です。家の外に置いて取り付けます。',
			'subs' => array(
				'oil'     => '石油給湯器',
				'ecofeel' => 'エコフィール（高効率石油給湯器）',
			),
		),
		'' => array(
			'ttl'  => '',
	...
	foreach ( $fuelgroups as $_gk => $_gd ) {

		$subs = array();
		$all  = array();
		foreach ( $_gd['subs'] as $_sk => $_sl ) {
			if ( empty( $bin[ $_sk ] ) ) continue;
			$subs[] = array(
				'ttl'   => $_sl,
				'maker' => null,
				'posts' => $bin[ $_sk ],
			);
			$all = array_merge( $all, $bin[ $_sk ] );
		}
		if ( ! $all ) continue;
